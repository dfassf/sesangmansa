...
async def cs_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/cs 커맨드 — 온디맨드 CS 노트."""
    await update.message.reply_text("CS 노트를 준비하고 있습니다... 잠시만 기다려주세요.")
    try:
        from app.bot.sender import send_briefing

        result = await send_briefing("cs_note", bot=context.bot)
        if result.get("error"):
            await update.message.reply_text("CS 노트 생성에 실패했습니다.")
    except Exception as exc:
        logger.error(f"온디맨드 CS 노트 실패: {exc}")
        await update.message.reply_text("CS 노트 생성 중 오류가 발생했습니다.")


async def expression_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/expression 커맨드 — 온디맨드 표현 학습."""
    await update.message.reply_text("오늘의 표현을 준비하고 있습니다... 잠시만 기다려주세요.")
    try:
        from app.bot.sender import send_briefing

        result = await send_briefing("expression", bot=context.bot)
        if result.get("error"):
            await update.message.reply_text("표현 노트 생성에 실패했습니다.")
    except Exception as exc:
        logger.error(f"온디맨드 표현 노트 실패: {exc}")
        await update.message.reply_text("표현 노트 생성 중 오류가 발생했습니다.")
